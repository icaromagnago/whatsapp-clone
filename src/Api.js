...
export default {
            ...
            if (doc.exists) {
                ...
                if (data.chats) {
                    ...
                    chats.sort((chatA, chatB) => {
                        if (chatA.lastMessageDate === undefined) {
                            return -1;
                        }
                        if (chatB.lastMessageDate === undefined) {
                            return -1;
                        }
                        if (chatA.lastMessageDate.seconds < chatB.lastMessageDate.seconds) {
                            return 1;
                        } else {
                            return -1;
                        }
    ...
    sendMessage: async (chatData, userId, type, body, users) => {
        ...
        users && users.forEach(async u => {
            let user = await db.collection('users').doc(u).get();
